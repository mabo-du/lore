...
from PyQt6.QtWidgets import (
    ...
    QStackedWidget, QSplitter, QPushButton, QLabel, QFileDialog, QMessageBox, QComboBox, QCheckBox
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.play_btn.setStyleSheet("background-color: #007acc; color: white; border-radius: 4px; padding: 6px;")
        ...
        self.metadata_form.generate_abstract_requested.connect(self._on_generate_abstract)
    ...
    def start_transcription(self):
        ...
        self.worker.segment_completed.connect(self.ner_worker.enqueue_segment, Qt.ConnectionType.QueuedConnection)
    ...
    def start_translation(self):
        if not self.transcript_model.get_transcript().segments:
            QMessageBox.warning(self, "Warning", "No transcript available to translate.")
            return
        ...
        self.stack.setCurrentIndex(1) # Switch back to status page to show translation progress
    ...
    def _on_translation_finished(self, transcript):
        ...
        QMessageBox.information(self, "Translation Complete", "The translation has finished successfully.")

    def _on_file_selected(self, path: Path):
        self.original_audio_path = path
        self.stack.setCurrentIndex(1) # Status page
        self.status_label.setText(f"Normalising audio (16kHz mono)...")
    ...
    def _update_time_label(self):
        ...
        self.time_label.setText(f"{pos_s // 60:02d}:{pos_s % 60:02d} / {dur_s // 60:02d}:{dur_s % 60:02d}")

    def _on_export(self, metadata: dict):
        save_path, _ = QFileDialog.getSaveFileName(
            self, "Save OHMS XML", str(self.original_audio_path.with_suffix(".xml")), "XML Files (*.xml)"
        )
        if save_path:
            try:
                OhmsExporter.export(self.transcript_model.get_transcript(), metadata, Path(save_path))
                QMessageBox.information(self, "Export Successful", f"Saved to {save_path}")
                ...
                try:
                    ...
                    idx.index_transcript(project_id, self.transcript_model.get_transcript())
                except Exception as e:
    ...
    def _on_bagit_export(self, metadata: dict):
        dir_path = QFileDialog.getExistingDirectory(
            self, "Select Output Directory for BagIt Package", str(self.original_audio_path.parent)
        )
        if dir_path:
            try:
                from lore_core.bagit_exporter import BagItPackager
                import uuid
                project_id = metadata.get("title", "Untitled").replace(" ", "_") + "_" + str(uuid.uuid4())[:8]
                ...
                QMessageBox.information(self, "Export Successful", f"BagIt package created at:\n{bag_dir}")
                ...
                QMessageBox.critical(self, "Export Error", f"Failed to export BagIt:\n{str(e)}")
