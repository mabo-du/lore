...
def test_index_and_search_keyword(search_index):
    ...
    t1.segments.append(
        Segment(
            start_ms=0, end_ms=1000, text="The quick brown fox jumps over the lazy dog."
        )
    )
    ...
    t2.segments.append(
        Segment(
            start_ms=0, end_ms=1000, text="Some unrelated text about space exploration."
        )
    )
    ...
    assert (
        "<b>brown</b>" in results[0]["snippet"] or "<b>fox</b>" in results[0]["snippet"]
    )
...
def test_index_and_search_semantic(search_index):
    ...
    t2.segments.append(
        Segment(
            start_ms=0, end_ms=1000, text="NASA launched a new rover to Mars today."
        )
    )
