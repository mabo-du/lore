...
class TranscriptionWorker(QThread):
    ...
    diarization_completed = pyqtSignal(list)   # Emits updated list of segments with speakers
    finished = pyqtSignal()
    error = pyqtSignal(str)

    def __init__(self, audio_path: Path, quality_tier: str = "Best Quality", 
                 enable_diarization: bool = False, use_pyannote: bool = False, hf_token: str = None, 
                 custom_vocab: str = "", parent=None):
    ...
    def run(self):
        try:
            self.status_changed.emit("Ensuring model is downloaded (this may take a while)...")
            ...
            for segment in engine.transcribe(self.audio_path, initial_prompt=self.custom_vocab):
            ...
            if self.enable_diarization:
                ...
                diarizer = DiarizationEngine(use_pyannote=self.use_pyannote, hf_token=self.hf_token)
