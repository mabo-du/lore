...
def test_full_workflow(qtbot, app_window, dummy_audio_file, mocker, tmp_path):
    ...
    mocker.patch(
        "lore_core.audio_classifier.AudioClassifyWorker.run",
        lambda self: self.finished.emit() if hasattr(self, "finished") else None,
    )
    ...
    mocker.patch(
        "ui.main_window.normalise",
        lambda in_path, out_path: shutil.copy2(in_path, out_path),
    )
    ...
    assert (
        model.data(model.index(0, 0), model.TextRole)
    ...
    mocker.patch(
        "PyQt6.QtWidgets.QFileDialog.getExistingDirectory", return_value=str(export_dir)
    )
