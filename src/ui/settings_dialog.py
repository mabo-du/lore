...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.radio_pyannote = QRadioButton("Pyannote 3.1 (High Accuracy, Requires Token)")
    ...
    def save_settings(self):
        if self.radio_pyannote.isChecked() and not self.token_input.text().strip():
            QMessageBox.warning(self, "Token Required", "Please enter a HuggingFace token to use Pyannote.")
            return
            
        self.settings.setValue("diarization/use_pyannote", self.radio_pyannote.isChecked())
        self.settings.setValue("diarization/hf_token", self.token_input.text().strip())
        self.settings.setValue("transcription/custom_vocab", self.vocab_input.text().strip())
        self.accept()
